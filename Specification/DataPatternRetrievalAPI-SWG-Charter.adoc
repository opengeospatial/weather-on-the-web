...
The Data Retrieval Pattern API Standards Working Group will standardise the APIs, defined with OpenAPI Version 3, to retrieve various commmon data patterns from a data cube. the dta patterns will include, but are not restricted to, data at a point in space and time, timeseries at a point, data along a trajectory, which may be 2, 3, or 4 dimensional, and covering a specified polygon.  
...
Much environmental data is truly Big Data, in that it cannot be readily copied and distributed in sensible timescales for many uses. By specifying precisely the data required in a convenient pattern familiar to the user, 
